...
// ----（以下、あなたの ingest コードをそのまま）----
function errStr(e){ if(!e) return 'unknown'; if(typeof e==='string') return e; if(e?.message) return e.message; try{ return JSON.stringify(e);}catch{ return String(e);} }
...
function parsePage($, baseUrl){
  const dateDiv={};
  $('div[class*="str_id-"]').each((_,div)=>{ const cls=$(div).attr('class')||''; const m=cls.match(/str_id-(\d+)/); if(m) dateDiv[m[1]]=$(div); });
  const rows=[];
...
function normalizeKey(s=''){ return s.replace(/[’＇`´‘]/g,"'").replace(/[“”＂]/g,'"').replace(/[‐‑‒–—―ーｰ]/g,'ー').replace(/[&＆]/g,'&').replace(/\s+/g,' ').trim(); }
...
function dedupeRows(rows){ const map=new Map(); for(const r of rows){ if(!r?.name) continue; const k=normalizeKey(r.name); if(!map.has(k)) map.set(k,r); else map.set(k, mergeRow(map.get(k), r)); } return [...map.values()]; }
  ...
  try{
    ...
    for (const p of PAGES){
      try{
        ...
        const park = parkByCode[p.code]; if(!park) throw new Error(`parks not found for code=${p.code}`);

        const upserts = rows.map(r=>({ park_id:park.id, name:r.name, tdr_url:r.url }));
        const { data:attrs, error:upErr } = await sb.from('attractions').upsert(upserts, { onConflict:'park_id,name' }).select('id,name');
        if (upErr) throw new Error(`attractions upsert: ${errStr(upErr)}`);

        const idByName = Object.fromEntries((attrs||[]).map(a=>[a.name,a.id]));
        const snaps = rows.map(r=>({ attraction_id:idByName[r.name], dpa_status:r.dpa, pp40_status:r.pp, status_operational:null, source:'netlify' })).filter(s=>!!s.attraction_id);
      ...
      }catch(e){ console.error(`ingest error ${p.code}:`, errStr(e)); }
    }

    // Background Function なので 202 を返すのが普通
    return { statusCode: 202, body: 'queued' };
  }catch(e){
    console.error('fatal', errStr(e));
    return { statusCode: 500, body: errStr(e) };
  }
}
